...
export class Forest extends Ecs.World {
    ...
    attach_stack(
        ...
        if (container) {
            ...
            if (window) {
                ...
                log.warn(
                    'attempted to attach window to stack that does not exist'
                );
            }
        } else {
    ...
    *iter(
    ...
    private readjust_fork_ratio_by_left(
        ...
        fork.set_ratio(left_length).measure(
            this,
            ext,
            fork.area,
            this.on_record()
        ...
        if (fork_c.area) {
            if (fork_c.is_horizontal()) {
                if ((movement & (DOWN | UP)) != 0) {
                    ...
                    if ((movement & LEFT) != 0) {
                        this.readjust_fork_ratio_by_left(
                            ext,
                            crect.width,
                            fork_c
                        );
    ...
    display_fork(
        ...
        fmt +=
            ' '.repeat((1 + scope) * 2) + `workspace: (${fork.workspace}),\n`;
        fmt +=
            ' '.repeat((1 + scope) * 2) +
            'left: ' +
